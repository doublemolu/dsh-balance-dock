...
export default {
  ...
  apply(ctx) {
    ...
    async function fetchBalanceText(apiKey) {
      ...
      // 清洗 Key：去掉 NUL/CR/LF 等会破坏 Windows 环境块或导致 spawn EINVAL 的字符
      const safeKey = String(apiKey || '').replace(/[\0\r\n]/g, '').trim()
      const policy = ctx.get('sandboxPolicy')
      const rawCwd = policy && typeof policy.workspaceRoot === 'string' ? policy.workspaceRoot : ''
      // cwd 交给 Node 自己决定（undefined = 进程当前目录），避免坏 cwd 触发 EINVAL
      const cwd = rawCwd.length > 0 ? rawCwd : undefined
      ...
      // 候选顺序：node.exe（真 exe）→ curl（真 exe）→ node（可能是 .cmd 壳，Node spawn 无法直接执行 .cmd，兜底）
      const nodeExe = await sub.resolveExecutable('node.exe').catch(() => null)
      const curlExe = await sub.resolveExecutable('curl').catch(() => null)
      const nodeCmd = await sub.resolveExecutable('node').catch(() => null)
      const attempts = []
      if (nodeExe !== null) attempts.push({ argv: [nodeExe, '-e', script], label: 'node.exe' })
      if (curlExe !== null) attempts.push({ argv: [curlExe, '-sS', '-H', 'Authorization: Bearer ' + safeKey, BALANCE_URL], label: 'curl' })
      if (nodeCmd !== null && nodeCmd !== nodeExe) attempts.push({ argv: [nodeCmd, '-e', script], label: 'node' })
      if (attempts.length === 0) throw new Error('PATH 中找不到可用的 node.exe / curl')
      let last
      for (const attempt of attempts) {
        ...
        } catch (e) { last = new Error(attempt.label + ' spawn 失败: ' + String((e && e.message) || e)); continue }
        let outcome
        try {
        ...
        last = new Error(attempt.label + ' exit ' + outcome.exitCode + (err ? ': ' + err.slice(0, 300) : ''))
      }
    ...
    if (ws !== undefined) {
      ...
      // 临时调试：暴露 spawn 参数（cwd / node 解析路径 / key 形状，不含 key 本身）
      ws.register({ kind: 'exact', path: '/dsh-balance/debug', handler: async (req, res) => {
        try {
          const sub = ctx.get('subprocess')
          const policy = ctx.get('sandboxPolicy')
          let nodePath = null
          let curlPath = null
          if (sub !== undefined) {
            try { nodePath = await sub.resolveExecutable('node') } catch (e) { nodePath = 'ERR:' + String((e && e.message) || e) }
            try { curlPath = await sub.resolveExecutable('curl') } catch (e) { curlPath = 'ERR:' + String((e && e.message) || e) }
          }
          const credentials = ctx.get('credentials')
          let keyShape = null
          if (credentials !== undefined) {
            const hit = await credentials.resolve('DEEPSEEK_API_KEY')
            keyShape = hit ? { len: hit.value.length, head: hit.value.slice(0, 6), source: hit.source } : 'NOT_CONFIGURED'
          }
          sendJson(res, {
            sub: sub !== undefined ? 'ok' : 'MISSING',
            policy: policy !== undefined ? 'ok' : 'MISSING',
            cwd: policy ? policy.workspaceRoot : null,
            nodePath,
            curlPath,
            keyShape,
          })
        } catch (e) { sendJson(res, { code: 'DEBUG_ERROR', error: String((e && e.message) || e) }, 500) }
      } })
