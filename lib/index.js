...
// 配置：$DSH_HOME/dsh-costometer.json（缺失时自动生成默认配置）
...
const DEFAULT_CONFIG = {
  ...
  prices: {              // 官方人民币价格（每百万 token），按峰谷时段分开（2026-08-17 起生效）
    'deepseek-v4-flash': {
      peak: { in: 3.0, inHit: 0.10, out: 9.0 },
      off: { in: 1.5, inHit: 0.05, out: 4.5 },
    },
    'deepseek-v4': {
      peak: { in: 3.0, inHit: 0.10, out: 9.0 },
      off: { in: 1.5, inHit: 0.05, out: 4.5 },
    ...
    'deepseek-chat': {
      peak: { in: 2.0, inHit: 0.20, out: 3.0 },
      off: { in: 1.0, inHit: 0.10, out: 1.5 },
    },
    'deepseek-reasoner': {
      peak: { in: 4.0, inHit: 1.0, out: 16.0 },
      off: { in: 2.0, inHit: 0.5, out: 8.0 },
    },
  },
}
...
const configPath = () => {
  ...
  return home === null ? null : home + '\\dsh-costometer.json'
}
...
// 仅保留新格式（peak/off）的价目；旧平铺格式（美元时代）直接丢弃，沿用默认新价目
function normalizePrices(prices) {
  const out = {}
  if (!prices || typeof prices !== 'object') return out
  for (const [model, entry] of Object.entries(prices)) {
    if (entry && typeof entry === 'object' && entry.peak && typeof entry.peak.in === 'number') out[model] = entry
  }
  return out
}
...
function loadConfig() {
  ...
  // 旧名迁移：若新文件不存在而旧文件存在，复制旧配置到新名
  if (!existsSync(path)) {
    const legacy = legacyConfigPath()
    if (legacy !== null && existsSync(legacy)) {
      try { copyFileSync(legacy, path) } catch (e) {}
    }
  }
  try {
    ...
    merged.prices = { ...DEFAULT_CONFIG.prices, ...normalizePrices((parsed && parsed.prices) || {}) }
    return merged
  } catch {
...
const isOffPeakWindow = (date) => {
  const t = date.getUTCHours() * 60 + date.getUTCMinutes()
  return t >= 990 || t < 30 // 16:30-00:30 UTC 为谷时段
}
...
export default {
  ...
  apply(ctx) {
    ...
    const estimateCost = (model, u, date) => {
      const cfg = config()
      const entry = cfg.prices[model] || cfg.prices['deepseek-chat'] || DEFAULT_CONFIG.prices['deepseek-chat']
      const p = isOffPeakWindow(date) ? (entry.off || entry.peak) : entry.peak
      const cny = ((u.input * p.in) + (u.cacheRead * p.inHit) + (u.cacheWrite * p.in) + (u.output * p.out)) / 1e6
      return { cny, usd: cny / cfg.fxCny }
    }
    ...
    async function foldSession(sid) {
      ...
      let last = null
      let foldError = null
      // 读取会话事件的三个候选：持久层原始读取（不经 Session.create 回放校验）→ sessionQuery → 实时会话
      let events = null
      const sp = ctx.get('sessionPersistence')
      if (sp !== undefined) {
        try {
          const insp = await sp.inspect(sid)
          if (insp && Array.isArray(insp.events)) events = insp.events
        } catch (e) { foldError = 'persistence:' + String((e && e.message) || e) }
      }
      if (events === null) {
        ...
        if (sq !== undefined) {
          try {
            ...
            if (snapshot && Array.isArray(snapshot.events)) events = snapshot.events
          } catch (e) { foldError = (foldError ? foldError + '; ' : '') + 'sessionQuery:' + String((e && e.message) || e) }
        }
      }
      if (events === null) {
        const sessions = ctx.get('sessions')
        const live = sessions && typeof sessions.get === 'function' ? sessions.get(sid) : undefined
        if (live && live.events !== undefined) {
          try { events = Array.isArray(live.events) ? live.events : Array.from(live.events.values ? live.events.values() : []) } catch (e) {}
        }
      }
      if (events !== null) {
        try {
          ...
          foldError = (foldError ? foldError + '; ' : '') + 'fold:' + String((e && e.message) || e)
        }
      } else if (foldError === null) {
        foldError = 'no event source available'
      }
      if (foldError !== null) console.error('[dsh-balance-dock] foldSession(' + sid + '):', foldError)
      const result = { total, last, foldError }
      foldCache.set(sid, { at: now, total, last, foldError })
    ...
    async function queryBalance() {
      ...
      try {
        ...
        const toppedUpNow = Number(info.topped_up_balance) || 0
        trackRecharge(toppedUpNow)
        return {
          ok: true,
          currency: String(info.currency || 'CNY'),
          total: Number(info.total_balance) || 0,
          granted: Number(info.granted_balance) || 0,
          toppedUp: toppedUpNow,
    ...
    const RATE_CURRENCIES = ['USD', 'EUR', 'JPY', 'KRW', 'RUB', 'TWD', 'GBP']
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (data && data.rates) {
              ...
              let okCount = 0
              for (const c of RATE_CURRENCIES) {
                if (typeof data.rates[c] === 'number' && data.rates[c] > 0) { rates[c] = data.rates[c]; okCount += 1 }
              }
              if (okCount > 0) {
    ...
    // ── 充值记录：本地追踪（官方 API 不提供充值历史；通过观测充值余额增长记录）──
    const DATA_PATH = (() => {
      const home = process.env.DSH_HOME || (process.env.USERPROFILE ? process.env.USERPROFILE + '\\.dsh' : null)
      return home === null ? null : home + '\\dsh-costometer-data.json'
    })()
    const LEGACY_DATA_PATH = (() => {
      const home = process.env.DSH_HOME || (process.env.USERPROFILE ? process.env.USERPROFILE + '\\.dsh' : null)
      return home === null ? null : home + '\\dsh-balance-dock-data.json'
    })()
    let rechargeState = null
    function loadRechargeState() {
      if (rechargeState !== null) return rechargeState
      if (DATA_PATH === null) { rechargeState = { recharges: [], lastSeenToppedUp: null }; return rechargeState }
      // 旧名迁移
      if (!existsSync(DATA_PATH) && LEGACY_DATA_PATH !== null && existsSync(LEGACY_DATA_PATH)) {
        try { copyFileSync(LEGACY_DATA_PATH, DATA_PATH) } catch (e) {}
      }
      try {
        const parsed = JSON.parse(readFileSync(DATA_PATH, 'utf8'))
        rechargeState = {
          recharges: Array.isArray(parsed && parsed.recharges) ? parsed.recharges : [],
          lastSeenToppedUp: (parsed && typeof parsed.lastSeenToppedUp === 'number') ? parsed.lastSeenToppedUp : null,
        }
      } catch {
        rechargeState = { recharges: [], lastSeenToppedUp: null }
      }
      return rechargeState
    }
    function saveRechargeState() {
      if (DATA_PATH === null || rechargeState === null) return
      try { writeFileSync(DATA_PATH, JSON.stringify(rechargeState, null, 2), 'utf8') } catch (e) {}
    }
    function trackRecharge(toppedUp) {
      const st = loadRechargeState()
      if (st.lastSeenToppedUp !== null && toppedUp > st.lastSeenToppedUp + 0.01) {
        st.recharges.push({ at: Date.now(), amount: round(toppedUp - st.lastSeenToppedUp, 2) })
      }
      st.lastSeenToppedUp = toppedUp
      saveRechargeState()
    }
    ...
    if (ws !== undefined) {
      ...
      ws.register({ kind: 'exact', path: '/dsh-balance/spend', handler: async (req, res) => {
        try {
          ...
          if (key !== null) {
            ...
            total = folded.total
            foldError = folded.foldError || null
      ...
      ws.register({ kind: 'exact', path: '/dsh-balance/recharge', handler: async (req, res) => {
        try {
          const st = loadRechargeState()
          const last = st.recharges.length > 0 ? st.recharges[st.recharges.length - 1] : null
          const total = st.recharges.reduce((s, r) => s + (typeof r.amount === 'number' ? r.amount : 0), 0)
          sendJson(res, { ok: true, last, total: round(total, 2), count: st.recharges.length })
        } catch (e) { sendJson(res, { ok: false, error: String((e && e.message) || e) }, 500) }
      } })
